//! Splits the window between the clock, the world map and the key hints, so
//! the clock stays readable from a full screen down to a small tiling pane.
...
pub fn split(area: Rect, with_map: bool) -> Screen {
    ...
    let body = Rect {
        height: area.height - u16::from(footer.is_some()),
        ..area
    };
...
mod tests {
        ...
        for width in (0..=320).step_by(7) {
            for height in (0..=100).step_by(3) {
                ...
                for with_map in [false, true] {
                    let screen = split(area, with_map);
                    let parts: Vec<Rect> = [Some(screen.clock), screen.map, screen.footer]
                        .into_iter()
                        .flatten()
                        .collect();
    ...
    fn hides_the_footer_in_tiny_windows() {
        assert!(split(Rect::new(0, 0, 80, 9), false).footer.is_none());
        assert!(split(Rect::new(0, 0, 19, 30), false).footer.is_none());
        assert_eq!(
            split(Rect::new(0, 0, 80, 24), false).footer,
    ...
    fn stacks_the_map_under_the_clock_in_tall_panes() {
        let screen = split(Rect::new(0, 0, 95, 50), true);
    ...
    fn puts_the_map_beside_the_clock_in_wide_panes() {
        let screen = split(Rect::new(0, 0, 200, 24), true);
        let map = screen.map.unwrap();
    ...
    fn drops_the_map_when_there_is_no_room() {
        assert!(split(Rect::new(0, 0, 40, 10), true).map.is_none());
        assert!(split(Rect::new(0, 0, 28, 40), true).map.is_none());
    }
